# UI part of futil.sample-configurator.sample-edit.sh
# This is not even a script, stupid and can't exist alone. It is purely
# ment for beeing included.

function print_sample-configurator.sample-edit_help() {
			cat <<EOF
Usage:
...
EOF
	...
	while getopts n:f:h OPTION; do
		...
		h)
			clear
			print_sample-configurator.sample-edit_help $0
			exit 0
			;;
		f)
			INOUT_FILE="$OPTARG"
		...
		?)
			...
			print_sample-configurator.sample-edit_help $0 1>&2
			exit 2
			;;
	...
	if [ ! $# -eq 0 ]; then
		echo "Syntax error: $(basename $0) takes o"\
		     "arguments" 1>&2
		echo 1>&2
		print_sample-configurator.sample-edit_help 1>&2
		exit 1
	fi
